...
function FileUploadComponent() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("http://172.17.0.2:5001/api/upload", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        if (result.status === "success") {
          ...
          if (result.file_id) {
            ...
            setDownloadLink(`http://172.17.0.2:5001/api/download_csv/${result.file_id}`);
  ...
  const handleDownloadCSV = async () => {
    ...
    try {
      ...
      if (csvDownloadResponse.ok) {
        ...
        const acknowledgmentResponse = await fetch(`http://172.17.0.2:5001/api/csv_acknowledgment/${fileId}`, {
          method: "POST",
        });
